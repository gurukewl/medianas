...
# medianas WPS (Wifi Protected Setup) Auto Connection on Startup
...
if [ "$(LANG=C && /sbin/ifconfig wlan0 | grep 'HWaddr\|ether' | wc -l)" -gt "0" -a "$(LANG=C && /sbin/ip addr show wlan0 | grep 'inet ' | grep -v '169.254' | wc -l)" -lt "1" ]; then
    ...
    sleep 1
    # Check if "update_config=1" needed in /opt/medianas/wpa_supplicant.conf for Autoconfig
    if [ "$(grep -i "update_config=1" /opt/medianas/wpa_supplicant.conf | wc -l)" -lt "1" ]; then
    	echo "update_config=1" >> /opt/medianas/wpa_supplicant.conf
    fi
    ...
    if [ "0" -lt "$(wpa_supplicant -h | grep nl80211 | wc -l)" ]; then
    	wpa_supplicant -B w -i wlan0 -c /opt/medianas/wpa_supplicant.conf
    else
    	wpa_supplicant -B w -D wext -i wlan0 -c /opt/medianas/wpa_supplicant.conf
    fi
    sleep 3
    ...
    VALIDENTRY=$(grep -i "^network=" /opt/medianas/wpa_supplicant.conf | wc -l)
    
    # wpa_supplicant.conf should be modified in last 20 seconds by WPS Config
    MODIFIED=$(( `date +%s` - `stat -L --format %Y /opt/medianas/wpa_supplicant.conf` ))
