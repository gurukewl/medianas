...
# probably /var/medianas/application/plugins/accesspoint/scripts/
...
if [ ! "$2" == "1" ]; then
	...
	if [ "$IS_EDIMAX" -gt "0" -a "$(lsb_release -r | grep '8.0' | wc -l)" -lt "1" -a "$RELEASE" != "stretch" ]; then 
	    ...
	    sudo cp /usr/sbin/hostapd /usr/sbin/hostapd-old;sudo cp -f /opt/medianas/hostapd-rtl /usr/sbin/hostapd
