...
class Passwordprotection_Setup extends Service
{
    ...
    private function _getProtectionInfo()
    {
        ...
        if (shell_exec('cat /var/www/medianas/public/.htaccess | grep "##AUTHENTICATION START"') != FALSE) {
    ...
    {
        // edit /var/www/medianas/.htaccess
        $user = $_REQUEST['user'];
        $pass = $_REQUEST['password'];
        $htaccess_code = "##AUTHENTICATION START\nAuthType Basic\nAuthName 'Protected medianas Login'\nAuthUserFile /var/www/medianas/.htpasswd\nRequire valid-user\n##AUTHENTICATION END";
        $script = array();
        $script[] = 'if [ $(dpkg -s apache2-utils | grep "install ok" | wc -l) -lt 1 ]; then apt-get update;echo "y" | apt-get install apache2-utils; fi';
        $script[] = "/bin/sed -i \"s@RewriteEngine on@" . str_replace("\n", "\\\\n", $htaccess_code) . "\\nRewriteEngine on@\" /var/www/medianas/public/.htaccess";
        $script[] = '/usr/bin/htpasswd -b -c /var/www/medianas/.htpasswd "' . $user . '" "' . $pass . '"';
        $script[] = 'echo "' . $htaccess_code . '" > /var/www/medianas/public/.htaccess_add';
    ...
    {
        // edit /var/www/medianas/.htaccess
        $script[] = '/bin/sed -i "/##AUTHENTICATION START/,/##AUTHENTICATION END/ { /##AUTHENTICATION END/"\'!\'" d; }" /var/www/medianas/public/.htaccess';
        $script[] = '/bin/rm /var/www/medianas/.htpasswd;/bin/rm /var/www/medianas/public/.htaccess_add';
        $this->writeDynamicScript($script);
