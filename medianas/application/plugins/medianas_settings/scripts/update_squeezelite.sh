...
if [ "$1" -gt "0" ]; then
	...
	sed -i 's/squeezelite=1/squeezelite=0/' /opt/medianas/autostart.conf
fi
...
ldconfig
pushd /tmp
git clone https://github.com/medianas/squeezelite;cd squeezelite
make -f Makefile.m2p
...
if [ "$1" -gt "0" ]; then
	...
	sed -i 's/squeezelite=0/squeezelite=1/' /opt/medianas/autostart.conf
fi

#### Finished ######

echo "Finished Update <b><a href='/plugins/medianas_settings/controller/Basic.php?action=reboot&redirecturl=/plugins/medianas_settings/controller/Squeezeplayer.php'>Please Click here to Reboot Now!</a></b>"
