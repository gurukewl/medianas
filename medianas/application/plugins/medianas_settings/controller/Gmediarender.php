...
class Gmediarender extends Service
{
    ...
    public function saveAudioDevice()
    {
        ...
        if ($this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'GMEDIARENDERER_ALSA_DEVICE', $value)) {
    ...
    {
        $output = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'GMEDIARENDERER_ALSA_DEVICE');
        if ($output) {
