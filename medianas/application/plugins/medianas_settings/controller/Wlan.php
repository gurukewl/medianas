...
class Wlan extends Service
{
    ...
    private $wpa_config = '/opt/medianas/wpa_supplicant.conf';
        ...
        while (! $searchSuccess && $attempts < 3) {
            ...
            if (preg_match('=wlan0.*Network is down=', $shellanswer) == 1) {
                ...
                $this->writeDynamicScript(array(
                    "wpa_supplicant -B w -D wext -i wlan0 -c /opt/medianas/wpa_supplicant.conf; sleep 3;"
                ));
                $shellanswer = shell_exec("LANG=C && sudo iwlist scan 2>&1");
            }
    ...
    private function _saveWirelessConfig()
    {
        ...
        if ($_GET['lanmac'] != '' && $this->view->lanmac != $_GET['lanmac'] && preg_match("=([0-9abcdefABCDEF]{2}:){5}[0-9abcdefABCDEF]{2}=", $_GET['lanmac'], $matches) == true) {
            if ($this->view->bootconfig) {
                ...
                shell_exec("sudo /opt/medianas/change_mac_address.sh");
        ...
        if ($ssid != '' && ($wlanstatus == false || $saveWiFi == true) && $_GET['wlan_configured'] != false) {
            ...
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'autoreconnect_wifi', '1');
            ...
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'autoreconnect_wifi', '0');
            ...
            if (! isset($this->view->eth0) && ($this->view->auto_accesspoint_mode == 1 || $_REQUEST['auto_accesspoint_mode'] == 1)) {
                ...
                $this->writeDynamicScript(array(
                    "sleep 3;/var/www/medianas/application/plugins/accesspoint/scripts/install_accesspoint.sh /var/www/medianas/application/plugins/accesspoint/scripts/ 1"
                ), false, true);
            }
        }
        ...
        if (isset($_REQUEST['wpsenabled']) && $wpsenabled == FALSE) {
            $this->writeDynamicScript(array(
                'sed -i "s@^#Start Accesspoint on Boot.*@if [ \"\$(LANG=C \&\& /sbin/ip addr show eth0 | grep \'inet \' | wc -l)\" -lt \"1\" ]; then sudo /opt/medianas/wps_config.sh; fi\n#Start Accesspoint on Boot if no network connection available@" /etc/rc.local'
            ));
        } elseif (! isset($_REQUEST['wpsenabled']) && $wpsenabled == TRUE) {
            $this->writeDynamicScript(array(
                'sed -i "s@.*sudo /opt/medianas/wps_config.sh; fi@@" /etc/rc.local'
            ));
        }
        ...
        if (isset($_REQUEST['auto_accesspoint_mode']) && $this->view->auto_accesspoint_mode != $_REQUEST['auto_accesspoint_mode']) {
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'auto_accesspoint_mode', 1);
            // Check and Install hostapd and dnsmasq
            $this->view->message[] = nl2br($this->writeDynamicScript(array(
                "/var/www/medianas/application/plugins/accesspoint/scripts/install_accesspoint.sh /var/www/medianas/application/plugins/accesspoint/scripts/ 1 onlyinstall"
            )));
        } elseif (! isset($_REQUEST['auto_accesspoint_mode']) && $this->view->auto_accesspoint_mode == 1) {
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'auto_accesspoint_mode', 0);
        }
    ...
    private function _getWirelessConfig()
    {
        ...
        // Auto Accesspoint
        $this->view->auto_accesspoint_mode = $this->getConfigFileParameter('/opt/medianas/options.conf', 'auto_accesspoint_mode');
        return true;
    }
    ...
    public function showHelpSidebar()
    {
        ...
        $helpSidebar['content'] = _('<ul><li>On this page you may set up and configure your WiFi and see details on the connected interfaces and their IP-Address.</li><li><a href="https://youtu.be/HsEBb_CIJ5A" target="blank">Watch a video showing WiFi setup with medianas</a></li><li>Step by Step: search for networks - choose you prefered network, activate the enable WiFi checkbox, enter the password and click save.</li></ul>');
        return true;
    }
    
    public function installWifiDriver(){
        $this->view->message[] = nl2br($this->writeDynamicScript(array(
            'wget http://fars-robotics.net/install-wifi -O /opt/medianas/install-wifi.sh',
            'chmod +x /opt/medianas/install-wifi.sh',
            '/opt/medianas/install-wifi.sh'
        )));
        return true;
    }
}
