...
class Squeezeplayer extends Service
{
    ...
    public function setOptionsDependingSoundcard($setsoundcard = '')
    {
        ...
        if (strpos($setsoundcard, 'CARD=sndrpiwsp') !== FALSE) {
            ...
            $script[] = 'su - ' . $user . ' -c \'/opt/medianas/wolfson/Playback_to_Headset.sh 2>&1\'';
            $script[] = 'su - ' . $user . ' -c \'/opt/medianas/wolfson/Playback_to_Lineout.sh 2>&1\'';
            $script[] = 'su - ' . $user . ' -c \'/opt/medianas/wolfson/SPDIF_playback.sh 2>&1\'';
    ...
    public function saveSqueezeliteCommandline()
    {
        ...
        if ($this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'SQUEEZELITE_PARAMETER', $value)) {
    ...
    {
        $output = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'SQUEEZELITE_PARAMETER');
        ...
        $this->view->use_usb_dac = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'USE_USB_DAC');
        return true;
    }
    ...
    public function installLADSPA()
    {
        ...
        $this->view->message[] = nl2br($this->writeDynamicScript(array(
            '/opt/medianas/install_ladspa.sh "' . $this->getHardwareInfo() . '"'
        )));
        return true;
    }
    ...
    public function setUseUSB_DAC()
    {
        if ($_GET['use_usb_dac'] == 1) {
            ...
            $script[] = 'sudo cp /opt/medianas/alsa_medianas_usb.conf /usr/share/alsa/alsa.conf.d';
            ...
            if ($this->getHardwareInfo() == 'Raspberry PI' && ! ($this->info->linux[0] == "Raspbian" && $this->info->linux[1] == "jessie")) {
                ...
                $script[] = 'sed -i "s@module \"Eq10\";@@" /usr/share/alsa/alsa.conf.d/alsa_medianas_usb.conf';
            }
            $this->writeDynamicScript($script);
        } else {
            $value = 0;
            $this->writeDynamicScript(array(
                'sudo rm /usr/share/alsa/alsa.conf.d/alsa_medianas_usb.conf'
            ));
        }
        
        if ($this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'USE_USB_DAC', $value)) {
    ...
    private function getAllLogs()
    {
        ...
        $out['AUDIOPLAYER CONFIG'] = shell_exec('cat /opt/medianas/audioplayer.conf');
    ...
    {
        $outfile = '/opt/medianas/cache/update_squeezelite.txt';
 ...
 *
 * @api {post} /plugins/medianas_settings/controller/Squeezeplayer.php Equalizer Settings
 * @apiName Equalizer Settings
 * @apiGroup Audioplayer API
 * @apiVersion 1.0.0
 * @apiDescription Fetch / Update / Reset Equalizer Settings on medianas Device.<br />
 *
 ...
 * @apiParam {String} use_equalizer Override medianas Setting wether to read Equalizer settings: must be set to "1", if Equalizer is disabled in web interface
