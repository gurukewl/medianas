...
class Basic extends Service
{
    ...
    public $timezonesfile = '/opt/medianas/timezones.txt';
    ...
    public function __construct()
    {
        ...
        if (isset($_REQUEST['action'])) {
            ...
            if ($_REQUEST['action'] == 'checkmedianasUpdate') {
                $this->checkmedianasUpdate();
            }
            if ($_REQUEST['action'] == 'checkmedianasBetaUpdate') {
                $this->checkmedianasUpdate('beta');
            }
            ...
            if ($_REQUEST['action'] == 'save') {
                ...
                $this->updatemedianasNetworkLookup();
    ...
    {
        $output = trim(shell_exec('cat /opt/medianas/playername.txt'), "\n");
    ...
    public function getLocale()
    {
        ...
        if (! file_exists($this->timezonesfile)) {
            $script[] = "find /usr/share/zoneinfo/right/ -type f | sed 's/\/usr\/share\/zoneinfo\/right\///' > /opt/medianas/timezones.txt";
            $this->writeDynamicScript($script);
    ...
    {
        $this->saveConfigFileParameter('/opt/medianas/options.conf', 'DEBUG_WEBINTERFACE', isset($_REQUEST['DEBUG_WEBINTERFACE']));
    ...
    public function updatePlayername($name = '')
    {
        ...
        if ($name != '') {
            ...
            if ($name != $this->view->playername) {
                // Playername anpassen
                $output = shell_exec('echo ' . $name . ' > /opt/medianas/playername.txt');
    ...
    public function resetFactoryDefaults()
    {
        ...
        $files = array(
            '/opt/medianas/playername.txt',
            '/etc/hostname',
            '/etc/network/interfaces',
            '/etc/fstab',
            '/boot/boot.scr',
            '/opt/medianas/wpa_supplicant.conf',
            '/opt/medianas/samba.conf',
            '/opt/medianas/options.conf', // TODO: .sav
            '/opt/medianas/audioplayer.conf' // TODO: .sav
        );
    ...
    public function getmedianasNetworkLookup()
    {
        return $this->view->medianasNetworkLookup = $this->checkAutostart('medianasNetworkLookup', true);
    }

    public function updatemedianasNetworkLookup()
    {
        if ($this->updateAutostart('medianasNetworkLookup', (bool) $_REQUEST['medianasNetworkLookup'], true)) {
            $this->view->message[] = _('medianas Network Player Lookup saved');
    ...
    {
        if ($this->saveConfigFileParameter('/opt/medianas/options.conf', 'removedonate', $removedonate)) {
            $this->loadViewHeader(true);
    ...
    {
        if ($this->saveConfigFileParameter('/opt/medianas/options.conf', 'email', $email)) {
            ...
            if ($this->checkLicense() != false) {
                ...
                if ($this->getHardwareInfo() == 'Raspberry PI') {
                    // Not Needed anymore
                    // $this->installPlugin('http://shop.medianas.com/media/downloadable/currentversion/raspberrysettings.tar');
                    // $this->parsePlugins();
                    ...
                    if (strpos($email, '@') === FALSE)
                        $this->view->message[] = _('<b>For premium indication in the medianas Forum</b>, you may <a href="https://www.medianas.com/en/getting-started/connect-your-license-code-to-your-email-address/" target="_blank">connect your premium-code with your email-address</a>. This is also useful if you lose your code or want to receive the medianas Magazine with tips on Setup and Hardware.');
                    $this->loadViewHeader(true);
                }
            } else {
                $this->view->message[] = _('Your license could not be validated. Did you choose the right eMail-Address that is registered as a customer at www.medianas.com? If you have a key for activation please recheck, that the key is entered correctly.');
            }
            return true;
        }
        return false;
     ...
     */
    public function checkmedianasUpdate($version = '')
    {
        ...
        // Check auf Update
        $file = file_get_contents('https://shop.medianas.com/media/downloadable/currentversion/version.txt');
        if ((float) $this->info->version < (float) $file) {
            $this->view->message[] = _('medianas update started');
            // Start Script -> Download Files for Webserver and /opt/medianas
            $shellanswer = shell_exec('sudo /opt/medianas/update_medianas.sh ' . $version);
            $this->view->message[] = $this->formatMessageOutput($shellanswer);
            if (strpos($shellanswer, 'inflating: /opt/medianas/list_devices.sh') !== FALSE && strpos($shellanswer, 'extracting: /var/www/medianas/application/config/version.txt') !== FALSE) {
                $this->view->message[] = _('UPDATE SUCCESSFUL');
                $this->view->message[] = _('medianas-Webinterface Restarted - this takes up to 15 seconds - Wait for 15 seconds and Reload Page to see Changes');
                // Reload apache as Deamon in Background!
                $this->view->message[] = '<a href="http://www.medianas.com/changelog-medianas/" target="blank"><b>' . _('Click here to see what is new (Changelog)!') . '</b></a>';
                $this->view->message[] = $this->writeDynamicScript(array(
            ...
            $this->view->message[] = _('medianas Website not available - maybe no internet connection? Please try a reboot to fix the internet connection.');
        } else {
            $this->view->message[] = _('medianas is up to date - no update required');
        }
    ...
    private function _checkPluginUpdates($version = 'currentversion')
    {
        ...
        for ($i = 0; $i < count($plugins['plugin']); $i ++) {
            ...
            if (isset($plugin['updateurl']) && $plugin['updateurl'] != '' && ! is_array($plugin['updateurl'])) {
                ...
                // Check for missing complete Path in UpdateURL (e.g. coming from other sources / Preinstalled) and add path to medianas Server
                // https://shop.medianas.com/media/downloadable/currentversion/$1.tar
                if(strpos($plugin['updateurl'], 'http') === FALSE){
                    $plugin['updateurl'] = 'https://shop.medianas.com/media/downloadable/currentversion/'. $plugin['updateurl'] .'.tar';
                }
                // Always use https for shop.medianas
                if(strpos($plugin['updateurl'], 'https') === FALSE && strpos($plugin['updateurl'], 'shop.medianas.com') !== FALSE){
                ...
                if ($version == 'beta') {
                    // PATH: http://shop.medianas.com/media/downloadable/beta/accesspoint.tar
                ...
                try {
                    ...
                    if (strpos($h[0], '200') !== FALSE) {
                        ...
                        if ($diff->m != 0 || $diff->d != 0 || $diff->h != 0 || $diff->i != 0) {
                            $output = $this->writeDynamicScript(array(
                                '/opt/medianas/install_plugin.sh ' . $plugin['updateurl']
                            ));
    ...
    private function resizeFS()
    {
        ...
        if (in_array($resizePart, array(
            ...
            $script = array(
                '/opt/medianas/expandfs.sh ' . $resizePart . ' > /opt/medianas/cache/resize-medianas-log.txt'
            );
            $this->view->message[] = nl2br($this->writeDynamicScript($script));
            $this->view->message[] = nl2br(shell_exec('cat /opt/medianas/cache/resize-medianas-log.txt'));
            if ($this->getHardwareInfo() == 'ODROID-XU3') {
            ...
            $this->view->message[] = "<b><a href='/plugins/medianas_settings/controller/Basic.php?action=reboot'>" . _("Please Click here to Reboot Now") . "!</a></b>";
        } elseif (in_array($resizePart, array(
            'mmcblk0p3'
        ))) {
            $this->view->message[] = _('Filesystem already expanded - Nothing to do!');
        } else {
            $this->view->message[] = _('No Resize possible - no valid partition found to expand. Contact medianas-Support to add support for further file-systems.');
        ...
        if ($this->getHardwareInfo() == 'Raspberry PI') {
            $this->hints = array(
                'Internet' => array(
                    ...
                    'message' => _('Connected'),
                    'shellcommand' => 'export LANG=C && ping -W 1 -c 1 www.medianas.com | grep "1 received" | wc -l'
                ),
                'CPU Load' => array(
                    'name' => 'CPU Load',
                    'max' => 2,
    ...
    {
        if ($this->getConfigFileParameter('/opt/medianas/options.conf', 'showHelpOnSidebar') == "1")
    ...
    public function showHelpSidebar()
    {
        ...
        $helpSidebar['content'] = _('<ul><li>Use this page to change the devicename, update medianas to the latest version, expand the filesystem on a new installation and install new addons.</li><li>To install and activate a new addon on the buttom of this page, you have to take 2 steps: first install the addon and second enable it.</li></ul>');
        if ($this->view->hints) {
