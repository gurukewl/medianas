...
class Shairport extends Service
{
    ...
    {
        $this->view->squeezebox_serverip = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'LMSIP');
        $this->view->ignore_running_kodi = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'IGNORE_KODI_SHAIRPORT');
        return true;
    }
    ...
    {
        if ($this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'IGNORE_KODI_SHAIRPORT', intval($_REQUEST['ignore_running_kodi']))) {
        ...
        if ($this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'SHAIRPORT_PARAMETER', $value)) {
        ...
        if ($_REQUEST['squeezebox_serverip'] != $this->view->squeezebox_serverip) {
            $this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'LMSIP', str_replace('http://', '', $_REQUEST['squeezebox_serverip']));
            $this->getParameters();
        }
    ...
    {
        $output = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'SHAIRPORT_PARAMETER');
