...
class Squeezeserver extends Service
{
     ...
     * This needs the sudoers.d rights for the script /opt/medianas/install_lms.sh
     */
    public function installLMS($lmsversion = '')
    {
        ignore_user_abort(true);
        set_time_limit(7200);
        $shellanswer = shell_exec("cat /opt/medianas/cache/install_lms.txt");
        if ($shellanswer != '') {
            ...
            $lastmodified_seconds = trim(shell_exec("expr $(date +%s) - $(date +%s -r /opt/medianas/cache/install_lms.txt)"), "\n");
            if ($lastmodified_seconds > 600 || (time() - 2 * 60 * 60) > strtotime(trim($started[0], '- '))) {
                ...
                shell_exec("rm /opt/medianas/cache/install_lms.txt");
            }
            ...
            $shellanswer = shell_exec("sudo /opt/medianas/install_lms.sh update " . $downurl . " > /dev/null &");
            ...
            $this->view->message[] = '<div id="msgprogress"></div><script type="text/javascript">setTimeout(function(){reloadprogress("msgprogress", "/plugins/medianas_settings/controller/Squeezeserver.php", 1)}, 3000);</script>';
            return true;
        }
    }
    ...
    {
        $this->view->delayedStartup = $this->getConfigFileParameter('/opt/medianas/options.conf', 'SQUEEZESERVER_DELAY_START');
        return true;
    }
        ...
        if ($seconds != $this->view->delayedStartup) {
            ...
            if (intval(trim(shell_exec('grep -i "SQUEEZESERVER_DELAY_START" /etc/rc.local | wc -l'))) == 0) {
                $this->writeDynamicScript(array(
                    'sed -i "s@^exit 0@SQUEEZESERVER_DELAY_START=\$(grep -a \"SQUEEZESERVER_DELAY_START\" /opt/medianas/options.conf | sed -n -e \'s/^[A-Z_]*\=//p\')\n	if [ \"\$SQUEEZESERVER_DELAY_START\" -gt \"0\" ]; then\n		sleep \$SQUEEZESERVER_DELAY_START;/etc/init.d/logitechmediaserver restart\n	fi\nexit 0@" /etc/rc.local'
                ));
            }
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'SQUEEZESERVER_DELAY_START', intval($seconds));
            $this->getDelayedStartup();
        }
        return true;
    }
    ...
    {
        $shellanswer = shell_exec("sudo /opt/medianas/install_lms.sh check 2>/dev/null");
    ...
    private function _pluginInstall($ajax = 0)
    {
        ...
        if ($_REQUEST['lmsplugin'] == 'googlemusic') {
            if ($ajax == 0) {
                if ($this->getProgressWithAjax('/opt/medianas/cache/install_googlemusic.txt', 1, 1, 30, $message = false, $url = false, $failmessage = false, $finishedText = 'Finished installing')) {
                    $shellanswer = $this->writeDynamicScript(array(
                        $this->scriptPath . "lms_plugin_googlemusic.sh >> /opt/medianas/cache/install_googlemusic.txt 2>&1 &"
                    ), false, true);
                }
            } else {
                $status = $this->getProgressWithAjax('/opt/medianas/cache/install_googlemusic.txt', 0, 1, 30);
                ...
                if (strpos($status, 'Finished installing') !== FALSE) {
                    ...
                    shell_exec('rm /opt/medianas/cache/install_googlemusic.txt');
                }
            }
        }
        return true;
    ...
    private function _saveSettings()
    {
        ...
        if (! file_exists('/var/www/medianas/public/cache')) {
            $this->writeDynamicScript(array(
                'mkdir /var/www/medianas/public/cache; chmod 777 /var/www/medianas/public/cache;'
            ));
            $this->view->message[] = _('Cachefolder created');
        }
        
        // Folders: tar -cf /opt/medianas/cache/squeezebox_server_settings.tar -C /var/lib/squeezeboxserver/ . --exclude=updates --exclude=templates -p
        $shellanswer = $this->writeDynamicScript(array(
            "tar -cf /var/www/medianas/public/cache/squeezebox_server_settings.tar -C /var/lib/squeezeboxserver/ . --exclude=updates --exclude=templates -p"
        ));
