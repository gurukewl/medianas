	...
	<br /> <input type="button"
		...
		<div id="wifiadvanced" style="max-height: 700px;">
			...
			<div>
				...
				<?php echo _("Wifi Protected Setup: Enable WPS in your Router and reboot medianas to automatically set up WiFi. Starts only, if no LAN-cable connected!") ?>
		    	<br /> <input type="button"
						value="<?php echo _("Save all settings") ?>" name="save"
						onclick="document.getElementById('action').value='save';submit();" />
					<br />
		    ...
		    <?php echo _("Set fixed LAN IP-Address") ?> (<?php echo $wlan->view->fixedinterface ?>): <input
					...
					<br /><?php echo _("(Useful, if your router sometimes changes the IP-Address of you medianas-device and you need a fixed IP-Address.)") ?>
		    <br />
					<br /> <input type="button"
						value="<?php echo _("Save all settings") ?>" name="save"
						onclick="document.getElementById('action').value='save';submit();" />
		    ...
		    <?php echo _("Activate to automatically start Accesspoint Mode during boot up, if no WiFi or LAN connection is available") ?> : <input
					...
					<br />
					<br /><?php echo _("This enables you to configure WiFi from your Smartphone - no need for any LAN-Connection. Useful, if you don't want to connect the medianas-device with a network cable for initial setup of WiFi.") ?>
		    <br />
					<br /><?php echo _("<b>HowTo</b> (assuming no user changes on Accesspoint Plugin page): Plug-In Power cable -> wait for 2 minutes -> choose <i>medianas</i> as WiFi network on your smartphone -> enter passphrase <i>mypasscode</i> -> point your browser to http://medianas and configure WiFi as normally"); ?>
		    <br />
					<br /> <input type="button"
						value="<?php echo _("Save all settings") ?>" name="save"
						onclick="document.getElementById('action').value='save';submit();" />
		    ...
		    <?php echo _("Configure LAN Mac-Address") ?>: <input type="text"
					...
					<br /><?php echo _("(Change only if there are more medianas-Devices in one network)") ?>
			<br />
					<br /> <input type="button"
						value="<?php echo _("Save all settings") ?>" name="save"
						onclick="document.getElementById('action').value='save';submit();" />
