 ...
 * 
 * Alle Scripte müssen in die sudoers Datei mit Rechten für die Gruppe www-data - visudo oder nano /etc/sudoers.d/medianas
 * 
 */
...
class Service
{
 ...
 // Readonly Filesystem
    public $autostartconf = '/opt/medianas/autostart.conf';

    public $dynamicScript = '/opt/medianas/dynamic_script.sh';
    ...
    public function __construct()
    {
        ...
        $this->info = new Info();
        if ($this->getConfigFileParameter('/opt/medianas/options.conf', 'DEBUG_WEBINTERFACE') == true || isset($_REQUEST['debug']) && $_REQUEST['debug'] == true) {
            $this->debug = true;
     ...
     *            Use /opt/medianas/autostart.conf for Autostart
     ...
     *            defines wether update-rc.d or the autostartconfigfile in /opt/medianas should be used
     */
    ...
    {
        $output = $this->shell_exec('cat /opt/medianas/playername.txt');
        $this->info->playername = $output;
        return $output;
    }

    /**
     * get a list of all medianas-Devices in local network
     * Needs package nmap to list devices responding on port 5002
     * uses /opt/medianas/list_devices.sh
     */
    public function getAllNetworkPlayers()
    {
        // Only do network search if activated in basic-settings
        if (! $this->checkAutostart('medianasNetworkLookup', true)) {
            return false;
        }
        // Caching in txt-file - reload every $reload
        $reload = 3600; // 1 hour
        
        $devices_txt = $this->shell_exec('cat /opt/medianas/cache/list_devices.txt');
    ...
    /**
     * get Current Version of medianas-Webinterface
     */
    ...
    {
        $this->info->removedonate = $this->getConfigFileParameter('/opt/medianas/options.conf', 'removedonate');
        return $this->info->removedonate;
    }

    /**
     * If an Addon needs a minimum Version of medianas
     * 
    ...
    public function checkMinVersion($minversion = 0)
    {
        ...
        if ((float) $this->info->version < $minversion && strpos($this->info->version, 'Beta') === FALSE) {
            $this->view->message[] = str_replace('$MINVERSION', $minversion, _('You need at least medianas Version $MINVERSION to run this addon. Please update medianas on Settings tab!'));
            return false;
        }
        return true;
    }
    ...
    public function checkForUpdate()
    {
        $this->getVersion();
        // Check for Update
        $file = $this->getExternalFile('https://shop.medianas.com/media/downloadable/currentversion/version.txt', 3);
        if (strpos($this->info->version, 'Beta') !== FALSE) {
            $this->view->message[] = _('You are running a Beta-Version of medianas!');
        } elseif ((float) $this->info->version < (float) $file) {
            $this->view->message[] = _('medianas update is available - start update on tab Settings / Reboot');
        }
        if ($file === false) {
    ...
    public function getReadOnlyFS()
    {
        if ($this->readonly_fs === 'unset') {
            if ($this->getConfigFileParameter('/opt/medianas/options.conf', 'readonly') == '1') {
    ...
    public function writeDynamicScript($script = '', $background = false, $daemon = false)
    {
        ...
        if ($daemon) {
            ...
            exec("php /opt/medianas/dynamicscriptdaemon.php {$command} >> /dev/null 2>&1 &");
    ...
    {
        $this->saveConfigFileParameter('/opt/medianas/options.conf', 'showHelpOnSidebar', $active);
        return true;
    }
    ...
    public function getSystemUser()
    {
        if (! $this->info->system_user) {
            $system_user = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'SYSTEM_USER');
            if ($system_user)
    ...
    {
        if ($local == true) {
            $license = $this->getConfigFileParameter('/opt/medianas/options.conf', 'license');
            ...
            if ($license == 1 || ($license != 0 && strtotime($license) > time())) {
                ...
                if (! $silent)
                    $this->view->message[] = _('No valid medianas-License for additional plugins and features found. Please enter a valid eMail-address on the Settings-page to verify that you are a customer of medianas and to access all features.');
                return false;
            }
        }
        $email = $this->getConfigFileParameter('/opt/medianas/options.conf', 'email');
        
        include_once '../application/model/CheckLicense.php';
        $this->view->licenseDate = $license;
        if ($license == 1 || ($license != 0 && strtotime($license) > time())) {
            $this->view->license = true;
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'license', $license);
            return true;
        } else {
            $this->view->license = false;
            $this->saveConfigFileParameter('/opt/medianas/options.conf', 'license', '0');
            return false;
        }
    }
    ...
    public function installPlugin($pathToPlugin = '', $autoenable = false, $position = false, $default = false, $basedir = '')
    {
        ...
        if ($pathToPlugin == '' && isset($_FILES['uploadedfile']) && $_FILES['uploadedfile']['tmp_name']) {
        	...
        	$uploaddir = '/var/www/medianas/public/addons/';
        ...
        if (! $uploadsuccess && ! preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $pathToPlugin)) {
            ...
            $output = nl2br($this->writeDynamicScript(array(
                '/opt/medianas/install_plugin.sh ' . $pathToPlugin . ' ' . $add_params . ' ' . $basedir
            )));
            ...
            if ($autoenable == true) {
                if (preg_match("=Installing Plugin ([a-zA-Z0-9 _-]*)=", $output, $match)) {
                    if (preg_match('=\$this-\>pluginname \= \_\(\'(.*)\'=', $this->shell_exec('grep -i \'$this->pluginname\' /opt/medianas/cache/newplugin/' . $match[1] . '/controller/Setup.php'), $namematch)) {
                        $pluginname = $namematch[1];
                        
                        $this->enablePlugin($pluginname, $position, $default, $basedir);
                        
                        $this->view->message[] = str_replace('$PLUGINNAME', _($pluginname), _('Plugin $PLUGINNAME activated and added to main navigation. You may change the position and visibility in the addon configuration on the <a href="/plugins/medianas_settings/controller/Basic.php#pluginconfigblock">settings page</a>.'));
                        if ($basedir == '')
    ...
    {
        $this->view->email = $this->getConfigFileParameter('/opt/medianas/options.conf', 'email');
    ...
    public function updateEqualizer($equalvalue, $user = false, $card = 'equal')
    {
        ...
        // return true;
        if ($this->saveConfigFileParameter('/opt/medianas/audioplayer.conf', 'USE_EQUALIZER', ($_REQUEST['use_equalizer'] == 1) ? 1 : 0)) {
            // Changes successful
        }
    ...
    {
        $this->view->use_equalizer = $this->getConfigFileParameter('/opt/medianas/audioplayer.conf', 'USE_EQUALIZER');
     ...
     */
    public function showLicenseMessage($every = 1)
    {
        if ($this->checkLicense(true, true) == true)
            return false;
        $name = 'medianas-Interface-Requests';
        if (isset($_COOKIE[$name])) {
            ...
            if ($req >= $every) {
                ...
                if (! isset($this->view->error[0]))
                    $this->view->error[] = _("medianas-License <a href='/plugins/medianas_settings/controller/Basic.php'>is not activated</a>. Not all functions available!");
            } else {
    ...
    public function resetNetworkPlayerList()
    {
        ...
        $this->shell_exec('rm /opt/medianas/cache/list_devices.txt');
        return true;
    }
        ...
        if (file_exists('/tmp/automatic_accesspoint_mode')) {
            ...
            if (! in_array(get_called_class(), array(
                ...
                if(get_called_class() == 'Basic' && ($_REQUEST['action'] == 'reboot' || $_REQUEST['action'] == 'shutdown'))
                    return true;
                $this->view->message[] = _('Device is started in Automatic Accesspoint Mode. For security, only WiFi configuration is possible to connect medianas to your local network.');
                unset($_REQUEST);
                unset($_GET);
                unset($_POST);
            }
