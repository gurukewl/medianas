...
class News extends Service
{
    ...
    public function getNews($position = 0)
    {
        ...
        if (file_exists($this->cachepath . $position . '.html') && (filemtime($this->cachepath . $position . '.html') > (time() - 86000))) {
            ...
            $image = $this->getConfigFileParameter('/opt/medianas/options.conf', 'specialimage');
            $this->newslocation = $this->getConfigFileParameter('/opt/medianas/options.conf', 'newslocation');
            if (! $this->newslocation) {
                // Set to Default
                $this->newslocation = 'https://www.medianas.com/wp-content/uploads/medianas/news/news.php';
                $this->saveConfigFileParameter('/opt/medianas/options.conf', 'newslocation', $this->newslocation);
            }
